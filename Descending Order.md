...
LOOP
    ...
    BGT LOOP2                   ; If R2 > R3 (i.e., already in descending order), jump to LOOP2
    STR R3, [R1]                ; Swap: store the larger value (R3) in the current position
    STR R2, [R1, #4]            ; Store the smaller value (R2) in the next position
